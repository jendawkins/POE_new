...
class SplineLearnerPOE_4D():
    ...
    def update_theta(self, states, theta2, ob):
        ...
        g2_flat = (states[:-1,:]*(states[:-1,:]@theta2[0])).flatten(order='F')

        mu_post = ((self.dt*bmat.T@np.linalg.inv(self.pvar*self.dt))@(
            Y_est) + (bmat.T@np.linalg.inv(self.poe_var))@(g2_flat))@sig_post
        
        if self.bypass_f2:
            sig_post = np.linalg.inv((1/self.theta_var)*np.eye(bmat.shape[1]) + \
                self.dt*bmat.T@np.linalg.inv(self.pvar*self.dt)@bmat*self.dt)
    ...
    def update_x(self, x, y, x0, betas, theta_2, ob):
        ...
        x1next = np.random.normal(y[0, :], np.sqrt(self.mvar[0][0]))
        xp[0, :] = x1next
